...
IplImage* otsu(IplImage* img)
{
	...
	for (t = 0; t < 256; t++)
	{
		...
		for (p = 0, p_num_b = 0; p < t+1; p++) // number of pixel
		{
			p_num_b += *(arr + p);
		}

		for (p = 0, p_wgt_b = 0; p < t + 1; p++) // weight
		{
			p_wgt_b += (double)(*(arr + p))/ (double)(m*n);
		}

		
		for (p = 0, p_avg_b = 0; p < t + 1; p++) // mean
		{
			if (p_num_b == 0)
			{
				p_avg_b = 0;
		...
		for (p = 0, p_var_b = 0; p < t + 1; p++) // variance
		{
			if (p_num_b == 0)
			{
				p_avg_b = 0;
		...
		for (p = t, p_num_f = 0; p < 256; p++) // number of pixel
		{
			p_num_f += *(arr + p);
		}

		for (p = t, p_wgt_f = 0; p < 256; p++) // weight
		{
			p_wgt_f += (double)*(arr + p)/ (double)(m*n);
		}

		for (p = t, p_avg_f = 0; p < 256; p++) // mean
		{
			if (p_num_f == 0)
			{
				p_avg_f = 0;
		...
		for (p = t, p_var_f = 0; p < 256; p++) // variance
		{
			if (p_num_f == 0)
			{
				p_avg_f = 0;
